...
const TrustSection = () => {
  return (
    <section className="py-4 md:py-6 bg-[#FFB599] text-slate-900">
      <div className="container mx-auto px-4">
        ...
        <div className="flex overflow-x-auto pb-4 gap-4 scrollbar-none snap-x">
          {trustPoints.map((point, index) => (
            <div
              key={index}
              className="w-[280px] min-w-[280px] max-w-[280px] md:w-[240px] md:min-w-[240px] md:max-w-[240px] flex-shrink-0 snap-start p-6 rounded-2xl bg-white/40 hover:bg-white/60 transition-colors duration-300 h-[300px] flex flex-col"
            >
              <div className="w-12 h-12 rounded-xl bg-white/60 flex items-center justify-center mb-4 text-slate-800">
                <point.icon className="w-6 h-6" />
              </div>
              <h3 className="font-display text-lg mb-2 font-bold">{point.title}</h3>
              <p className="text-sm text-slate-800 font-medium mt-auto">
                {point.description}
              </p>
            </div>
          ))}
